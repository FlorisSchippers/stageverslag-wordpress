 ...
 * @since   3.7.0
 */
final class Burst_WP_Session extends Recursive_ArrayAccess implements Iterator, Countable {
	/**
	...
	protected function __construct() {
		if ( isset( $_COOKIE[BURST_WP_SESSION_COOKIE] ) ) {
			$cookie = stripslashes( $_COOKIE[BURST_WP_SESSION_COOKIE] );
	...
	protected function set_expiration() {
		$this->exp_variant = time() + (int) apply_filters( 'burst_wp_session_expiration_variant', 24 * 60 );
		$this->expires = time() + (int) apply_filters( 'burst_wp_session_expiration', 30 * 60 );
	}
	...
	protected function set_cookie() {
		setcookie( BURST_WP_SESSION_COOKIE, $this->session_id . '||' . $this->expires . '||' . $this->exp_variant , $this->expires, COOKIEPATH, COOKIE_DOMAIN );
	}
